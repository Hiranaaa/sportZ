...
<a href="{{ route('courts.show', $court->slug ?? '') }}" class="group block">
    <div class="card-hover rounded-2xl overflow-hidden">
        {{-- Image Container --}}
        <div class="relative aspect-video overflow-hidden">
            <img
                src="{{ $court->image_url ?? asset('images/courts/default.jpg') }}"
                alt="{{ $court->name ?? 'Lapangan' }}"
            ...
            <div class="absolute top-3 left-3">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full bg-white/90 dark:bg-navy-900/90 backdrop-blur-sm text-gray-900 dark:text-white shadow-sm">
                    @switch($court->sport_type ?? 'futsal')
                        @case('futsal') ⚽ Futsal @break
                        @case('badminton') 🏸 Badminton @break
                        @case('basket') 🏀 Basket @break
                        @case('tenis') 🎾 Tenis @break
                        @case('voli') 🏐 Voli @break
                        @default 🏟️ {{ ucfirst($court->sport_type ?? 'Sport') }}
                    @endswitch
                </span>
            </div>
            {{-- Favorite Button --}}
        ...
        <div class="p-5">
            ...
            <div class="flex items-center gap-1.5 mt-1.5 text-sm text-gray-500 dark:text-gray-400">
                ...
                <span class="line-clamp-1">{{ $court->location ?? 'Jakarta Selatan' }}</span>
            </div>
            ...
            <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-100 dark:border-white/5">
                ...
                <div>
                    <span class="text-xl font-heading font-bold text-gray-900 dark:text-white">
                        Rp {{ number_format($court->price_per_hour ?? 0, 0, ',', '.') }}
                    </span>
                ...
                <div class="flex items-center gap-1.5">
                    <div class="flex items-center gap-0.5">
                        @for($i = 1; $i <= 5; $i++)
                            @if($i <= floor($court->average_rating ?? 0))
                    ...
                    <span class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ number_format($court->average_rating ?? 0, 1) }}</span>
                </div>
            </div>
        </div>
    </div>
